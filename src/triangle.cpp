...
Triangle::Triangle(Vector c, Vector b, Vector a, Texture* t):Plane(Vector(0,0,0), t, 0., 0., 0., 0., 0.){
   ...
   // The basis (right, up, vect) is complete here, so the cached inverse must
   // be built before the first solveLocal() call rather than at the end.
   updateInverse();
   Vector np = solveLocal(a-c);
   textureY = np.y;
   thirdX = np.x;
...
double Triangle::getIntersection(Ray ray){
   ...
   if(time==inf) 
      return time;
   Vector dist = solveLocal(ray.point+ray.vector*time-center); 
...
bool Triangle::getLightIntersection(Ray ray, double* fill){
   ...
   Vector dist = solveLocal(ray.point+ray.vector*r-center);
